...
from reranker import warm_up_reranker
from store import (
    add_chunks,
    delete_source,
    search as vector_search,
    stats,
    warm_up_vector_components,
)
...
def warm_up_local_components() -> None:
    """在服务 ready 前只读预热本地 embedding、HNSW 与 reranker。"""
    total_started = time.perf_counter()
    try:
        vector_timings = warm_up_vector_components()
        logger.info(
            "warmup stage=embedding duration_ms=%.1f",
            vector_timings["embedding_ms"],
        )
        logger.info(
            "warmup stage=chroma-hnsw status=%s duration_ms=%.1f",
            vector_timings["hnsw_status"],
            vector_timings["hnsw_ms"],
        )

        reranker_started = time.perf_counter()
        reranker_ready = warm_up_reranker()
        logger.info(
            "warmup stage=reranker status=%s duration_ms=%.1f",
            "warmed" if reranker_ready else "vector-only",
            (time.perf_counter() - reranker_started) * 1000,
        )
    except Exception:
        logger.exception(
            "local warm-up failed duration_ms=%.1f",
            (time.perf_counter() - total_started) * 1000,
        )
        raise

    logger.info(
        "local warm-up complete duration_ms=%.1f",
        (time.perf_counter() - total_started) * 1000,
    )
...
async def lifespan(app: FastAPI):
    await asyncio.to_thread(warm_up_local_components)
    yield
...
def query(req: QueryRequest):
    ...
    如果没有检索到可靠段落，不会调用回答 LLM，并按原始问题语言返回
    “资料中找不到”说明。中文/含中文的混合问题返回中文，英文问题返回英文。
...
def search(req: SearchRequest):
    ...
    返回经过向量召回、阈值过滤和 Cross-Encoder 重排的段落数组；
    distance 保留原始 cosine 向量距离（越小越相似），最终数组顺序以重排结果为准。
    没有可靠命中时返回空数组。
...
async def upload_document(file: UploadFile = File(...)):
    ...
    支持格式：.pdf  .txt  .md
    大小限制：由 MAX_UPLOAD_BYTES 配置，默认 10 MiB（边读边检查）。

    同名文件再上传会先删除旧 chunks 再入库，保证不残留。

    错误码：
    - 400: 路径穿越 / 扫描版 PDF / 空文件 / 文件读取失败
    - 413: 文件超过 MAX_UPLOAD_BYTES
    - 415: 不支持的文件类型
    """
    # --- 安全文件名 -----------------------------------------------------------
