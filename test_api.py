...
class TestSearch:
    def test_valid_query_returns_results_without_llm(self, client):
        ...
        with (
            ...
            body = resp.json()
            assert len(body) == 2
            assert body[0]["source"] == "sample_handbook.pdf"
...
class TestAskRewrite:
    ...
    def test_system_prompt_refusal_follows_original_question_language(self):
        """拒答不硬编码中文，英文/中文都跟随原始问题语言。"""
        from ask import SYSTEM_PROMPT

        assert (
            "If the context does not contain the answer, state that the answer cannot be "
            "found in the provided documents, using the same language as the original "
            "question, and stop."
        ) in SYSTEM_PROMPT
        assert 'say exactly: "资料中找不到这个问题的答案。"' not in SYSTEM_PROMPT
    ...
    def test_contains_chinese(self):
        ...
        assert contains_chinese("这个 employee ID 是多少？")  # 混合也算中文问题
...
class TestAskResponseLanguage:
    @pytest.mark.parametrize(
        ("question", "expected_answer"),
        [
            ("知识库里有什么？", "知识库是空的，请先上传文档。"),
            (
                "What is in the knowledge base?",
                "The knowledge base is empty. Please upload documents first.",
            ),
        ],
    )
    def test_empty_knowledge_base_fallback_follows_question_language(
        self, question, expected_answer
    ):
        with patch(
            "ask.get_knowledge_base_stats",
            return_value={"chunks": 0, "sources": []},
        ):
            from ask import ask

            assert ask(question) == {"answer": expected_answer, "sources": []}

    @pytest.mark.parametrize(
        ("question", "expected_answer"),
        [
            ("实习期是多久？", "资料中找不到这个问题的答案。"),
            (
                "How long is the internship?",
                "I couldn't find the answer to this question in the available documents.",
            ),
        ],
    )
    def test_no_relevant_results_fallback_follows_question_language(
        self, question, expected_answer
    ):
        with (
            patch(
                "ask.get_knowledge_base_stats",
                return_value={"chunks": 5, "sources": ["test.pdf"]},
            ),
            patch("ask.rewrite_query", return_value="How long is the internship?"),
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "How long is the internship?",
                "Answer language: English. You MUST answer in English.",
            ),
            (
                "实习期包含 remote work 吗？",
    ...
    def test_final_generation_receives_original_question_language_instruction(
        self, question, expected_instruction
    ):
        with (
            patch(
                "ask.get_knowledge_base_stats",
            ...
            patch("ask.rewrite_query", return_value="Does the internship include remote work?"),
            patch("ask.retrieve", return_value=[FAKE_HIT]),
            patch("ask.generate_answer", return_value="answer") as mock_generate,
        ):
            from ask import ask

            ask(question)

            final_prompt = mock_generate.call_args.args[0]
            assert expected_instruction in final_prompt
            assert f"Question: {question}" in final_prompt
