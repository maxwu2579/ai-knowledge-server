...
class TestStoreSearchRerank:
    def test_overfetches_then_sends_at_most_10_unique_candidates_to_reranker(self):
        import store
        ...
        collection = fake_collection(docs, metas, dists, ids, count=15)
        with (
        ...
        assert collection.last_n_results == 15  # 库不足 20 条时取全部 raw candidates
        assert len(cands_arg) == 10  # 去重后仍只给 reranker 最多 Top-10
    ...
    def test_duplicate_candidates_do_not_consume_rerank_recall_positions(self):
        import store

        docs = ["duplicate"] * 12 + ["supervisor evidence"] + [f"tail{i}" for i in range(7)]
        metas = [{"source": "appointment.pdf", "page": 1}] * 20
        dists = [0.40 + i * 0.01 for i in range(20)]
        ids = [f"id{i}" for i in range(20)]
        collection = fake_collection(docs, metas, dists, ids, count=20)

        with (
            patch("store.get_collection", return_value=collection),
            patch("store.rerank", side_effect=lambda _q, candidates, top_k=None: candidates[:top_k])
            as mock_rerank,
        ):
            store.search("Who supervises the intern?", top_k=5)

        candidates = mock_rerank.call_args.args[1]
        assert collection.last_n_results == store.RAW_CANDIDATE_LIMIT == 20
        assert [candidate["text"] for candidate in candidates[:2]] == [
            "duplicate",
            "supervisor evidence",
        ]
        assert len(candidates) == 9

    def test_dedup_identity_includes_source_page_and_text(self):
        import store

        docs = ["same text", "same text", "same text", "same text"]
        metas = [
            {"source": "a.pdf", "page": 1},
            {"source": "a.pdf", "page": 1},
            {"source": "a.pdf", "page": 2},
        ...
        candidates = mock_rerank.call_args.args[1]
        assert [(c["source"], c["page"], c["text"]) for c in candidates] == [
            ("a.pdf", 1, "same text"),
            ("a.pdf", 2, "same text"),
            ("b.pdf", 1, "same text"),
        ]
